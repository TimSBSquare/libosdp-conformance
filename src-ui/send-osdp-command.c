...
{ /* main for send-osdp-command */
  ...
  if (param_verbosity > 1) fprintf(stderr, "arguments: %s\n", arguments);
  strcpy(tag, "cmd=MON-");
  if (0 == strncmp(tag, arguments, strlen(tag))) {
    printf("<HTML><HEAD><TITLE>libosdp Monitor Console</TITLE>");
    printf(
        "<META HTTP-EQUIV=\"REFRESH\" CONTENT=\"3;URL=/open-osdp-MON.html\">");
    printf("</HEAD><BODY>");
    strcpy(command, arguments + strlen(tag));
    if (0 == strcmp(command, "stop")) system("sudo -n killall open-osdp");
  };
  strcpy(tag, "cmd=CP-");
  if (0 == strncmp(tag, arguments, strlen(tag))) {
    ...
    printf(
        ...
        "CONTENT=\"1;URL=/osdp-conformance-CP.html\">");
#else
    printf(
  ...
  if (0 == strncmp(tag, arguments, strlen(tag))) {
    ...
    printf(
        "<META HTTP-EQUIV=\"REFRESH\" "
        "CONTENT=\"1;URL=/osdp-conformance-PD.html\">");
    printf("</HEAD><BODY>");
