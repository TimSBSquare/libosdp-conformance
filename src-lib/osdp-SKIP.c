void skip_conformance_tests(OSDP_CONTEXT* ctx, OSDP_INTEROP_ASSESSMENT* oconf)
...
{ /* skip_conformance_tests */
  ...
  oconf->cmd_max_rec.test_status = OCONFORM_SKIP;
  ...
  oconf->packet_size_stress_cp.test_status = OCONFORM_SKIP;
  oconf->rep_capas_consistent.test_status = OCONFORM_SKIP;
  ...
  oconf->resp_ostatr_range.test_status = OCONFORM_SKIP;
