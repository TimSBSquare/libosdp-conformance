...
{ /* background */
  ...
  if (context->role EQUALS OSDP_ROLE_CP) {
    current_length = 0;
    status =
        send_message(context, OSDP_POLL, p_card.addr, &current_length, 0, NULL);
  };
...
{
  int i;
  unsigned short int mask_bit;
  int results;

  results = 0;
  mask_bit = 1;
  for (i = 0; i < length; i++) {
    if (mask_bit & value) results++;
    mask_bit = mask_bit * 2;
  };
  results = results & 1;
  if (!sense) {
      ...
      */
        ...
        case PARMV_ROLE:
          if (strcmp(last_content, "CP") == 0)
            if (strcmp(last_content, "PD") == 0) context.role = OSDP_ROLE_PD;
...
{ /* fasc_n_75_to_string */
  ...
  status = 0;
  {
    ...
    sprintf(ret_string,
            "Agency Code: %lld System Code: %lld Card Number: %lld Expiration: "
            "%lld",
            v1, v2, v3, v4);
  };
...
{ /* next_sequence */
  ...
  if (ctx->last_response_received != OSDP_NAK)
    ...
    if (ctx->last_nak_error EQUALS OO_NAK_CHECK_CRC) do_increment = 0;
  };
  ...
  if (do_increment) {
    ...
    ctx->next_sequence++;
    if (ctx->next_sequence > 3) ctx->next_sequence = 1;
  } else {
    if (ctx->verbosity > 2)
...
int osdp_timeout(OSDP_CONTEXT* ctx, struct timespec* last_time_ex)
...
{ /* osdp_timeout */
  ...
  if (status_posix == -1) ctx->last_errno = errno;
...
int parse_json(OSDP_CONTEXT* ctx)
...
{ /* parse_json */
  ...
  if (found_field) {
    ...
    strcpy(vstr, json_string_value(value));
    sscanf(vstr, "%d", &i);
    if (i != 0) ctx->disable_certificate_checking = i;
  };
  ...
  if (found_field) {
    ...
    strcpy(vstr, json_string_value(value));
    if (0 EQUALS strcmp(vstr, "DEFAULT")) ctx->enable_secure_channel = 2;
  };
...
int read_config(OSDP_CONTEXT* ctx)
...
{ /* read_config */
  ...
  if (p_card.value_len EQUALS 26) {
    ...
    //               1+1      8           3+5         7 1
    unsigned short int cardholder;
    unsigned short int facility;
    unsigned short int parity1;
    unsigned short int parity2;
    unsigned int value;
...
{ /* send_bio_read_template */
  ...
  current_length = 0;
  status = send_message(ctx, OSDP_BIOREAD, p_card.addr, &current_length,
                        sizeof(param), param);
  if (ctx->verbosity > 2) fprintf(stderr, "Request bio read\n");
  return (status);

} /* send_bio_read_template */

int send_comset(OSDP_CONTEXT* ctx, unsigned char pd_address,
                unsigned char new_address, char* speed_string)
...
{ /* send_comset */
  ...
  status = send_message(ctx, OSDP_COMSET, pd_address, &current_length,
                        sizeof(param), param);
...
{ /* send_message */
  ...
  *current_length = 0;
  ...
  status =
      osdp_build_message(test_blk,        // message itself
                         current_length,  // returned message length in bytes
                         command, true_dest, next_sequence(context),
  ...
  if (status EQUALS ST_OK) {
    // if (context->verbosity > 3)
    {
      ...
      if (context->role EQUALS OSDP_ROLE_CP) parse_role = OSDP_ROLE_PD;
      status_monitor =
          osdp_parse_message(context, parse_role, &m, &returned_hdr);
    ...
    if (command EQUALS OSDP_POLL) context->cp_polls++;
  ...
  if (status EQUALS ST_OK) context->last_command_sent = command;
