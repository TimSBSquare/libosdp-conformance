...
{ /* initialize */
  ...
  if (status EQUALS ST_OK) {
    ...
    if (context.role EQUALS OSDP_ROLE_CP) fprintf(stderr, "Role: CP\n");
    if (context.role EQUALS OSDP_ROLE_PD) fprintf(stderr, "Role: PD\n");
...
{ /* init_tls_server */
  ...
  if (status EQUALS ST_OK) {
    ...
    gnutls_certificate_set_x509_trust_file(
        x509_cred, "/opt/open-osdp/etc/ca_keys.pem", GNUTLS_X509_FMT_PEM);
    gnutls_certificate_set_verify_function(x509_cred,
                                           _verify_certificate_callback);

    status_tls = gnutls_certificate_set_x509_key_file(
        x509_cred, config.cert_file, config.key_file, GNUTLS_X509_FMT_PEM);
    if (status_tls < 0) status = ST_OSDP_TLS_NOCERT;
  };
  if (status EQUALS ST_OK) {
    generate_dh_params();
    gnutls_priority_init(&priority_cache,
                         ...
                         "PERFORMANCE:%SERVER_PRECEDENCE:+PSK", NULL);
...
*/
    ...
    setsockopt(listen_sd, SOL_SOCKET, SO_REUSEADDR, (void*)&optval,
               sizeof(int));
    status_sock =
        bind(listen_sd, (struct sockaddr*)&sa_serv6, sizeof(sa_serv6));
    if (status_sock EQUALS - 1) status = ST_OSDP_TLS_BIND_ERR;
  };
  if (status EQUALS ST_OK) {
    status_sock = listen(listen_sd, 1024);
    if (status_sock EQUALS - 1) status = ST_OSDP_TLS_LISTEN_ERR;
    ;
  };
  if (status EQUALS ST_OK) {
  ...
  if (status EQUALS ST_OK) {
    ...
    status_sock = fcntl(sd, F_SETFL, fcntl(sd, F_GETFL, 0) | O_NONBLOCK);
...
*/
  ...
  */
              if ((osdp_timeout(&context, &last_time_check_ex)) ||
                ...
                if (context.authenticated) status = background(&context);
                request_immediate_poll = 0;
              };
            };
          };
          ...
          if (status EQUALS ST_OK) {
            ...
            if (!context.authenticated) {
              ...
              if (passphrase_length < plmax) {
                ...
                if (0 EQUALS memcmp(current_passphrase, specified_passphrase,
                                    plmax))
                  context.authenticated = 1;
              };
#endif
            ...
            if (context.authenticated) {
              ...
              while (!done) {
                if (buffer[i] != C_SOM) {
                  ...
                  request_immediate_poll =
                      0;  // saw an SOM, so normal incoming message
...
{ /* send_osdp_data */
  ...
  if (context->verbosity > 9) {
    ...
    for (i = 0; i < lth; i++) {
      ...
      if (7 EQUALS(i % 8)) fprintf(context->log, "\n");
    };
    if (7 != (lth % 8)) fprintf(context->log, "\n");
  };
 ...
 */
int _verify_certificate_callback(
    gnutls_session_t session) { /* _verify_certificate_callback */
  unsigned int status;
  int ret, type;
  const char* hostname;
  gnutls_datum_t out;
...
/* The following demonstrate two different verification functions,
  ...
  if (context.disable_certificate_checking) return 0;
