...
{ /* process_command */
    ...
    switch (command) {
      case OSDP_CMD_CP_SEND_POLL:
        ...
        status = send_message(context, OSDP_POLL, p_card.addr, &current_length,
                              0, NULL);
        if (context->verbosity > 2) fprintf(stderr, "Polling\n");
        break;
      ...
      case OSDP_CMD_COMSET: {
        ...
        param[3] = 0;
        param[4] = 0;
        status = send_message(context, OSDP_COMSET, 0x7f, &current_length,
                              sizeof(param), param);
  ...
  */
  ...
  if ((!processed) && (context->current_menu EQUALS OSDP_MENU_TOP)) {
    switch (command) {
      // breech-loaded cases.  by convention these occur at the top of the old
      // menu structure for transitional purposes.
      ...
      case OSDP_CMDB_DUMP_STATUS:
        ...
        fprintf(stderr, "  Timeout %02d(%d.) Dump %d Debug %d.\n",
                m_idle_timeout, p_card.poll, m_dump, context->verbosity);
        fprintf(stderr,
                " PwrRpt %d Special-1 %d\nCP Polls %d; PD Acks %d NAKs %d "
                "CsumErr %d\n",
                context->power_report, context->special_1, context->cp_polls,
                context->pd_acks, context->sent_naks, context->checksum_errs);
