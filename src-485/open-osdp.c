...
{ /* initialize */
  ...
  strcpy(tag, "PD");
  if (context.role EQUALS OSDP_ROLE_CP) strcpy(tag, "CP");
  if (context.role EQUALS OSDP_ROLE_MONITOR) strcpy(tag, "MON");
...
{ /* main for open-osdp */
...

  {
    ...
    if (ufd != -1) {
      ...
      if (status_socket != -1) {
        ...
        if (status_socket != -1) status_socket = listen(ufd, 0);
      };
    };
  };
  ...
  while (!done) {
    ...
    status_select =
        pselect(scount, &readfds, &writefds, &exceptfds, &timeout, &sigmask);
    ...
    if (status_select EQUALS 0) {
      ...
      if (osdp_timeout(&context, &last_time_check_ex)) {
        ...
        if (status EQUALS ST_OK) status = background(&context);
      };
    };
        ...
        if (status_io < 1) {
          ...
          if (osdp_buf.next < sizeof(osdp_buf.buf)) {
            ...
            if (osdp_buf.next EQUALS 1) {
              if (!(osdp_buf.buf[0] EQUALS C_SOM)) osdp_buf.next = 0;
  ...
  if (status != ST_OK) fprintf(stderr, "open-osdp return status %d\n", status);
