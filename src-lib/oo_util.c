...
unsigned int web_color_lookup[16] = {
    0x000000, 0xFF0000, 0x00FF00, 0x008080, 0x444444, 0x550101,
    0x660101, 0x770101, 0x0000FF, 0x010101, 0x010101, 0x010101,
...
int osdp_build_message(unsigned char* buf, int* updated_length,
                       ...
                       int data_length, unsigned char* data, int secure)
...
{ /* osdp_build_mesage */
  ...
  *p;
  ...
  if (context.role EQUALS OSDP_ROLE_PD) p->addr = p->addr | 0x80;
  ...
  if (role EQUALS OSDP_ROLE_PD) {
    switch (command) {
      default:
        ...
        // can't really check here because of that switch statement after the
        // call, hasn't all been migrated here.
        break;

      case OSDP_CHLNG:
        status = ST_OSDP_CMDREP_FOUND;
        m->data_payload = m->cmd_payload + 1;
        if (ctx->verbosity > 2) strcpy(tlogmsg2, "osdp_CHLNG");
      ...
      case OSDP_POLL:
        ...
        m->data_payload = NULL;
        m->data_length = 0;
        if (ctx->verbosity > 2) strcpy(tlogmsg2, "osdp_POLL");
        if (osdp_conformance.conforming_messages < PARAM_MMT)
          osdp_conformance.conforming_messages++;
        break;
      ...
      case OSDP_SCRYPT:
        status = ST_OSDP_CMDREP_FOUND;
        m->data_payload = m->cmd_payload + 1;
        if (ctx->verbosity > 2) strcpy(tlogmsg2, "osdp_SCRYPT");
  ...
  if (role EQUALS OSDP_ROLE_CP) {
    switch (command) {
      default:
        ...
        m->data_payload = m->cmd_payload + 1;
        if (ctx->verbosity > 2) strcpy(tlogmsg2, "\?\?\?");
      ...
      case OSDP_ACK:
        ...
        m->data_payload = NULL;
        m->data_length = 0;
        if (ctx->verbosity > 2) strcpy(tlogmsg2, "osdp_ACK");
        ctx->pd_acks++;
...
{ /* parse_message */
  ...
  *p;
  ...
  if (msg_check_type EQUALS 0) {
    ...
    osdp_conformance.checksum.test_status = OCONFORM_EXERCISED;
  } else {
    m->check_size = 2;
    m_check = OSDP_CRC;
  };
  ...
  if (m->lth >= (m->check_size + sizeof(OSDP_HDR))) {
    ...
    if (m->lth < msg_lth)
      ...
      osdp_conformance.multibyte_data_encoding.test_status = OCONFORM_EXERCISED;
  };
  if (status != ST_OK) {
    if (status != ST_MSG_TOO_SHORT) {
      fprintf(context->log,
              "parse_message did not clear the header.  msg_data_length %d. "
              "msg_check_type 0x%x m->check_size %d. m->lth %d. msg_lth %d "
              "status %d.\n",
  ...
  if (status EQUALS ST_OK) {
    ...
    if (context->verbosity > 3)
      ...
      sprintf(tlogmsg, "A:%02x L:%d. C:%02x", (0x7f & p->addr), msg_lth,
              p->ctrl);

    // must start with SOM
    if (p->som != C_SOM) status = ST_MSG_BAD_SOM;
  };
  if (status EQUALS ST_OK) {
    ...
    if (msg_scb EQUALS 0) {
      ...
      osdp_conformance.scb_absent.test_status = OCONFORM_EXERCISED;
    } else {
      {
        int i;
    ...
    if ((context->verbosity > 4) ||
        ((m->msg_cmd != OSDP_POLL) && (m->msg_cmd != OSDP_ACK))) {
      char dirtag[1024];
      int i;
      unsigned char* p1;
      char tlogmsg[1024];
      ...
      if (!m_dump) {
        if (msg_data_length) {
          fprintf(context->log, "  DATA (%d. bytes):\n    ", msg_data_length);
        };
    ...
    if (status != ST_OSDP_CMDREP_FOUND) {
      if (status != ST_OK)
        fprintf(stderr,
                "Status %d Unknown command? (%02x), default msg_data_length "
                "was %d\n",
      ...
      switch (returned_hdr->command) {
        default:
          ...
          if (context->verbosity > 2) strcpy(tlogmsg2, "\?\?\?");
        ...
        case OSDP_BIOREAD:
          m->data_payload = NULL;
          msg_data_length = 0;
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_BIOREAD");
        ...
        case OSDP_BUSY:
          m->data_payload = NULL;
          msg_data_length = 0;
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_BUSY");
        ...
        case OSDP_MFG:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_MFG");
          break;
        ...
        case OSDP_NAK:
          ...
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          context->sent_naks++;
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_NAK");
          break;
        ...
        case OSDP_BUZ:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_BUZ");
          break;
        ...
        case OSDP_CAP:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_CAP");
          break;
        ...
        case OSDP_COM:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_COM");
          break;

        case OSDP_COMSET:
          m->data_payload = m->cmd_payload + 1;
          msg_data_length = 5;
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_COMSET");
          break;
        ...
        case OSDP_ID:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_ID");
          break;

        case OSDP_ISTAT:
          m->data_payload = NULL;
          msg_data_length = 0;
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_ISTAT");
          break;
        ...
        case OSDP_ISTATR:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_ISTATR");
          break;
        ...
        case OSDP_KEYPAD:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_KEYPAD");
          break;
        ...
        case OSDP_LED:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_LED");
          break;

        case OSDP_LSTAT:
          m->data_payload = NULL;
          msg_data_length = 0;
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_LSTAT");
          break;
        ...
        case OSDP_LSTATR:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_LSTATR");
          break;
        ...
        case OSDP_MFGREP:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_MFGREP");
          break;

        case OSDP_OSTAT:
          m->data_payload = NULL;
          msg_data_length = 0;
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_OSTAT");
          break;
        ...
        case OSDP_OSTATR:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_OSTATR");
          break;
        ...
        case OSDP_OUT:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_OUT");
          break;
        ...
        case OSDP_PDCAP:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_PDCAP");
        ...
        case OSDP_PDID:
          ...
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          // ASSUMES NO SECURITY
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_PDID");
        ...
        case OSDP_RAW:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_RAW");
          break;

        case OSDP_RSTAT:
          m->data_payload = NULL;
          msg_data_length = 0;
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_RSTAT");
          break;
        ...
        case OSDP_RSTATR:
          ...
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_RSTATR");
          break;
        ...
        case OSDP_TEXT:
          ...
          msg_data_length = p->len_lsb + (p->len_msb << 8);
          msg_data_length = msg_data_length - 6 - 2;  // less hdr,cmnd, crc/chk
          if (context->verbosity > 2) strcpy(tlogmsg2, "osdp_TEXT");
          break;
      };
    };  // bolt-on for PD/CP switch statements.
    // if it was found it's ok
    if (status EQUALS ST_OSDP_CMDREP_FOUND) status = ST_OK;
    ...
    if (m->check_size EQUALS 2) {
      ...
      if (parsed_crc != wire_crc) status = ST_BAD_CRC;
      ...
      // experimental
      if (m->lth == 7) wire_cksum = *(m->cmd_payload + 1 + msg_data_length);
    ...
    if ((context->verbosity > 2) || (m_dump > 0)) {
      ...
      sprintf(tlogmsg2, " S:%02x Ck %x Sec %x CRC %04x", msg_sqn,
              msg_check_type, msg_scb, wire_crc);
...
*/
  ...
  if ((status != ST_OK) && (status != ST_MSG_TOO_SHORT)) {
    if (strlen(logmsg) > 0) fprintf(context->log, "%s\n", logmsg);

    // if parse failed report the status code
    if ((context->verbosity > 3) && (status != ST_MONITOR_ONLY)) {
      fflush(context->log);
      fprintf(context->log, "Message input parsing failed, status %d\n",
              status);
    };
  };
  return (status);
    ...
    switch (msg->msg_cmd) {
      case OSDP_BIOREAD:
        ...
        // we don't actually DO a biometrics read at this time, so NAK it.
        {
          ...
          status = send_message(context, OSDP_NAK, p_card.addr, &current_length,
        ...
        osdp_conformance.cmd_bioread.test_status = OCONFORM_EXERCISED;
        current_length = 0;
        status = send_message(context, OSDP_ACK, p_card.addr, &current_length,
                              0, NULL);
        context->pd_acks++;
        break;
      ...
      case OSDP_CAP: {
        unsigned char osdp_cap_response_data[3 * (14 - 1)] = {
          1,
          2,
          OOSDP_CFG_INPUTS,  // 8 inputs, on/of/nc/no
          2,
          2,
          8,  // 8 outputs, on/off/drive
          3,
          1,
          0,  // 1024 bits max
          4,
          1,
          8,  // on/off only, 8 LED's
          5,
          1,
          1,  // audible annunciator present, claim on/off only
          6,
          1,
          1,  // 1 row of 16 characters
          ...
          10,
          0xff & OSDP_BUF_MAX,
          (0xff00 & OSDP_BUF_MAX) >> 8,  // rec buf max
          11,
          0xff & OSDP_BUF_MAX,
          (0xff00 & OSDP_BUF_MAX) >> 8,  // largest msg
          12,
          0,
          0,  // no smartcard
          13,
        ...
        status = ST_OK;
        current_length = 0;
        status = send_message(context, OSDP_PDCAP, p_card.addr, &current_length,
                              sizeof(osdp_cap_response_data),
                              osdp_cap_response_data);
        osdp_conformance.cmd_pdcap.test_status = OCONFORM_EXERCISED;
        osdp_conformance.rep_device_capas.test_status = OCONFORM_EXERCISED;
        if (context->verbosity > 2)
      ...
      case OSDP_COMSET: {
        ...
        *(unsigned short int*)(osdp_com_response_data + 1) =
            9600;  // hard-code to 9600 BPS
        status = ST_OK;
        current_length = 0;
        status = send_message(context, OSDP_COM, p_card.addr, &current_length,
                              sizeof(osdp_com_response_data),
                              osdp_com_response_data);
        if (context->verbosity > 2) {
      ...
      case OSDP_CHLNG: {
        ...
        // make sure this PD was enabled for secure channel (see
        // enable-secure-channel command)
        ...
        if (!nak) {
          ...
          if (status EQUALS ST_OK) {
            ...
            // client ID
            memcpy(ccrypt_response.client_id, context->vendor_code, 3);
            ccrypt_response.client_id[3] = context->model;
            ccrypt_response.client_id[4] = context->version;
            memcpy(ccrypt_response.client_id + 5, context->serial_number, 3);

            // RND.B
            memcpy((char*)(context->rnd_b), "abcdefgh", 8);
            memcpy((char*)(ccrypt_response.rnd_b), (char*)(context->rnd_b),
                   sizeof(ccrypt_response.rnd_b));
            printf("fixme: RND.B\n");
            ...
            status = send_secure_message(
                context, OSDP_CCRYPT, p_card.addr, &current_length,
      ...
      case OSDP_ID: {
        ...
        status = ST_OK;
        current_length = 0;
        status = send_message(context, OSDP_PDID, p_card.addr, &current_length,
                              sizeof(osdp_pdid_response_data),
                              osdp_pdid_response_data);
        osdp_conformance.cmd_id.test_status = OCONFORM_EXERCISED;
        osdp_conformance.rep_device_ident.test_status = OCONFORM_EXERCISED;
      ...
      case OSDP_ISTAT:
        status = ST_OK;
        {
          unsigned char osdp_istat_response_data[OOSDP_CFG_INPUTS];
          ...
          osdp_conformance.resp_input_stat.test_status = OCONFORM_EXERCISED;
          current_length = 0;
          status = send_message(
              context, OSDP_ISTATR, p_card.addr, &current_length,
              sizeof(osdp_istat_response_data), osdp_istat_response_data);
          if (context->verbosity > 2) {
            sprintf(logmsg,
                    "Responding with OSDP_ISTAT (hard-coded all zeroes)");
            fprintf(context->log, "%s\n", logmsg);
          };
        };
        break;
  ...
  */
          ...
          *led_ctl;
          ...
          for (i = 0; i < count; i++) {
            fprintf(context->log, "[%02d] Rdr %d LED %d Tcmd %d Pcmd %d\n", i,
                    led_ctl->reader, led_ctl->led, led_ctl->temp_control,
            ...
            if (led_ctl->reader EQUALS 0)
              if (led_ctl->perm_control EQUALS 1) {
                ...
                // for conformance tests 3-10-1/3-10-2 we specifically look for
                // LED 0 Color 1 (Red) or Color 2 (Green)

                if (led_ctl->perm_on_color EQUALS 1)
                  osdp_conformance.cmd_led_red.test_status = OCONFORM_EXERCISED;
      ...
      case OSDP_LSTAT:
        status = ST_OK;
        {
          ...
          osdp_lstat_response_data[1] =
              context->power_report;  // report power failure
          current_length = 0;
          status = send_message(
              context, OSDP_LSTATR, p_card.addr, &current_length,
  ...
  if (context->role EQUALS OSDP_ROLE_CP) {
    ...
    switch (msg->msg_cmd) {
      ...
      case OSDP_ISTATR:
        ...
        count = oh->len_lsb + (oh->len_msb << 8);
        count = count - 8;
        sprintf(tlogmsg, "count: %d. %02x %02x %02x %02x", count,
                *(0 + msg->data_payload), *(1 + msg->data_payload),
                *(2 + msg->data_payload), *(3 + msg->data_payload));
      ...
      case OSDP_COM:
        ...
        if (context->verbosity > 2) {
          fprintf(stderr,
                  "osdp_COM: Addr %02x Baud (m->l) %02x %02x %02x %02x\n",
                  *(0 + msg->data_payload), *(1 + msg->data_payload),
                  *(2 + msg->data_payload), *(3 + msg->data_payload),
                  *(4 + msg->data_payload));
        };
      ...
      case OSDP_MFGREP: {
        ...
        *mmsg;
        ...
        sprintf(tlogmsg,
                ...
                mmsg->MpdSizeTotal, mmsg->MpdOffset, mmsg->MpdFragmentSize,
                mmsg->Reply_ID);
    ...
    */
        ...
        if (status == ST_OK) {
          ...
          if ((context->next_in + mmsg->MpdFragmentSize) ==
              context->total_len) {
